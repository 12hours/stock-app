...
public class ProductRouteTest {
    ...
    ArgumentCaptor<ProductForm> integerArgumentCaptor;

    @Before
    public void setUp() {
        integerArgumentCaptor = ArgumentCaptor.forClass(ProductForm.class);
    }
    ...
    public void getAllProductsTest() throws Exception {
        ...
        expectedHashMap.put("products", TestData.products());
    ...
    public void getProductByIdTest() throws JsonProcessingException {
        ...
        Exchange response = template.send(RouteNames.PRODUCT_ROUTE, exchange);
        HashMap<String, Object> expectedHashMap = new HashMap<>();
        expectedHashMap.put("product", TestData.products().get(0));
        assertEquals(objectMapper.writeValueAsString(expectedHashMap), response.getIn().getBody());
    }
    ...
    public void saveProductTest() throws Exception {
        ...
        Exchange response = template.send(RouteNames.PRODUCT_ROUTE, exchange);
        Mockito.verify(productServiceMock).saveProduct(integerArgumentCaptor.capture());
        assertEquals(integerArgumentCaptor.getValue(), new ProductForm("testProduct", BigDecimal.valueOf(100), Integer.valueOf(1)));
